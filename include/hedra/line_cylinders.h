...
namespace hedra
{
                                   ...
                                   Eigen::MatrixXd& TC)
    {
        ...
        int NewColorSize=(PerVertex ? NewV.rows() : NewF.rows());
        TC.resize(NewColorSize,3);
        
        MatrixXd PlanePattern(NumSides,2);
        for (int i=0;i<NumSides;i++){
            std::complex<double> CurrRoot=exp(2*M_PI*Complex(0,1)*(double)i/(double)res);
        ...
        for (int i=0;i<P1.rows();i++){
            ...
            for (int j=0;j<res;j++){
                ...
                if (PerVertex){
                    TC.row(v1)<<CylinderColors.row(i);
                    TC.row(v2)<<CylinderColors.row(i);
                }
                ...
                if (!colorPerVertex){
                    TC.row(2*res*i+2*j)<<CylinderColors.row(i);
                    TC.row(2*res*i+2*j+1)<<CylinderColors.row(i);
                }
            }
        }
