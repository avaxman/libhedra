...
namespace hedra
{
  ...
  inline Eigen::RowVector3d edge_inner_average_linear_blend(const Eigen::RowVector3d& v1,const Eigen::RowVector3d& e1, const Eigen::RowVector3d& e2,const Eigen::RowVector3d& v2){
    return (e1+e2)/2.0;
    
  }
  
  inline Eigen::RowVector3d vertex_boundary_cc_linear_blend(const Eigen::RowVector3d& a,
                                                            const Eigen::RowVector3d& b,
                                                            const Eigen::RowVector3d& p,
                                                            const Eigen::RowVector3d& c,
                                                            const Eigen::RowVector3d& d){
    return ((a+d)/8.0+p*3.0/4.0);
  }
  
  inline Eigen::RowVector3d vertex_boundary_cc_moebius_blend(const Eigen::RowVector3d& a,
                                                             const Eigen::RowVector3d& b,
                                                             const Eigen::RowVector3d& p,
                                                             const Eigen::RowVector3d& c,
                                                             const Eigen::RowVector3d& d){
  ...
  inline bool vertex_star_cc(const Eigen::RowVector3d& central,
                             const Eigen::MatrixXd& vertexStar,
                             const bool isBoundaryVertex,
                             Eigen::MatrixXd newCentral;
                             Eigen::MatrixXd& facePoints,
                             Eigen::MatrixXd& edgePoints,
                             std:function<const RowVector3d& v0,const MatrixXd& vr, const RowVector3d& c, const double r>& original2Canonical,
                             std:function<const RowVector3d& v0,const MatrixXd& vr, const RowVector3d& c, const double r>& canonical2Original){
    
    int N=vr.rows();
    facePoints.conservativeResize(isBoundaryVertex ? vertexStar.rows()-1 : vertexStar.rows());
    facePoints.setZero();
    edgePoints.conservativeResize(vertexStar.rows(),3);
    edgePoints.setZero();
    
    MatrixXd canonicalStar=original2canonical(central, vertexStar, c,r);
    
    
    
    
    for (int i=0;i<faceVertices.rows();i++)
      candidateFacePoints.row(i)<<facePoint;
    
    return facePoint;
  }
  
  
  for (int i=(isBoundaryVertex ? 1 : 0);i<(isBoundaryVertex ? vr.rows()-1 : vr.rows());i++){
    edgePoints.row(i)=(v0+vr.row(i)+facePoints.row(i)+facePoints.row((i+N-1)%N))/4.0;
  }
  
  if (isBoundaryVertex){
    edgePoints.row(0)=(v0+vr.row(0))/2.0;
    edgePoints.row(edgePoints.rows()-1)=(v0+vr.row(vr.rows()-1))/2.0;
  }
  
  RowVector3d newv0;
  if (!isBoundaryVertex){
    RowVector3d F = facePoints.colwise().mean();
    RowVector3d E = edgePoints.colwise().mean();
    newv0=(F+E*4.0-F*2.0+ (double)(N-3)*v0)/(double)N;
  } else {
    if (vr.rows()>2)
      newv0=v0*3.0/4.0+(vr.row(0)+vr.row(vr.rows()-1))/8.0;
      else  //an ear
        newv0=v0;
        }
  return newv0;
...
inline RowVector3d computeCCMoebiusVertexEdgePoint(const RowVector3d& v0, const MatrixXd& vr, const bool isBoundaryVertex, const RowVector3d& c, const double r, const MatrixXd& ringCandidateFacePoints, MatrixXd& ringEdgePoints){
  
  
  MatrixXd canonvr=original2canonical(v0, c,r,vr);
  MatrixXd canonRingEdgePoints, canonRingFacePoints=original2canonical(v0, c,r,ringCandidateFacePoints);
  
  RowVector3d linearNewv0=computeCCVertexEdgePoint(c, canonvr, isBoundaryVertex, c, r, canonRingFacePoints, canonRingEdgePoints);
  RowVector3d MoebiusNewv0=canonical2original(v0, c, r, linearNewv0);
  ringEdgePoints=canonical2original(v0, c, r, canonRingEdgePoints);
