...
Eigen::MatrixXd origV, deformV, V;
...
double currWinZ;
hedra::ComplexMoebiusData complexMoebiusData;
...
void choose_current_mesh(){
  ...
  switch(viewingMode){
    ...
    case QC_ERROR:
      V=(showDeformed ? deformV : origV);
      T=origT;
      break;
      
    case MC_ERROR:
    case IA_ERROR:
      V=(showDeformed ? edgeDeformV : edgeOrigV);
      T=edgeOrigT;
      break;
  }
}
...
bool UpdateCurrentView(igl::viewer::Viewer & viewer)
{
  ...
  switch(viewingMode){
    case FACE_CONCYCLITY:
      hedra::concyclity(V,D,F,scalarMeasure);
    ...
    case QC_ERROR:
      ...
      break;
    case MC_ERROR:
      hedra::mc_error(origV, deformV, D, F, scalarMeasure);
      scalarMeasure/=MCSensitivity;
      break;
    case IA_ERROR:
      hedra::ia_error(origV, deformV, D, F, scalarMeasure);
...
bool mouse_move(igl::viewer::Viewer& viewer, int mouse_x, int mouse_y)
{
  ...
  hedra::complex_moebius_deform(complex_moebius_data, bc,1000, isExactMC, isExactIAP, deformV);
  hedra::edge_mesh(deformV,D,F,EV, EF, deformEdgeV, edgeT,edgeTE);
...
bool mouse_down(igl::viewer::Viewer& viewer, int button, int modifier)
{
  ...
  hedra:complex_moebius_init(b, isExactMC, isExactIAP, rigidityFactor)
...
bool key_down(igl::viewer::Viewer& viewer, unsigned char key, int modifiers)
{
  switch(key)
  {
    ...
    case '2': viewingModes=(viewingModes+1)%4;
      UpdateCurrentView();
      break;
  }
  return false;
...
int main(int argc, char *argv[])
{
  ...
  hedra::polygonal_read_OFF(TUTORIAL_SHARED_PATH "/bar2d.off", V, D, F);
