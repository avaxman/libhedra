...
struct MeanCurvatureError{
public:
    MeanCurvatureError(double* _pH, double* _factor):pH(_pH), factor(_factor){}
    ~MeanCurvatureError(){};
    ...
    bool operator()(const T* const pi,
        ...
        Eigen::Matrix< T, 1, 4 > Nj=QMultT<T>(wj-wi, QInvT<T>(wk-wj));
        Eigen::Matrix< T, 1, 4 > Nl=QMultT<T>(wl-wk, QInvT<T>(wi-wl));
        Eigen::Matrix< T, 1, 4 > Commutator=QMultT(Nj,Nl)-QMultT(Nl,Nj);
        Eigen::Matrix< T, 1, 4 > HError=Commutator-(T)(*pH)*(wi-wk);
        for (int i=0;i<4;i++)
            residuals[i]=(T)((*factor)*HError(i));
...
public:
              ...
              const Eigen::MatrixXi& inFaceTriads)
    {
        ...
        /*for (int i=0;i<OrigVq.rows();i++){
            ceres::CostFunction* cost_function=new AutoDiffCostFunction<UnitNormError, 1, 3 >(new UnitNormError(&UnitFactor));
            problem->AddResidualBlock(cost_function,
                                      NULL, // TODO: update with coefficients somehow,
                                      CurrSolution+3*OrigVq.rows()+3*i);
        }
        ...
        for (int i=0;i<F.rows();i++){
            ...
            problem->AddResidualBlock(cost_function,
                                       NULL, // TODO: update with coefficients somehow,
                                       CurrSolution+3*OrigVq.rows()+3*OrigVq.rows()+3*i);
        }*/
